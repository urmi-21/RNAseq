...
library(SRAdb)
#update sradb file
#getSRAdbFile() #uncomment to download file
...
#update db
#getSQLiteFile() #uncomment to download latest geodb
...
all_sra_data <-
  dbGetQuery(
    sra_con,
    paste(
      "select * from sra WHERE (taxon_id IN ('txID') AND UPPER(platform) = 'ILLUMINA' AND UPPER(library_strategy) = 'RNA-SEQ' AND UPPER(library_layout) LIKE 'PAIRED%' AND UPPER(library_source) = 'TRANSCRIPTOMIC')",
      sep = " "
    )
  )
...
fname="US_AT_removedtabs.csv"
write.csv(filterfinal, fname, row.names=FALSE)
